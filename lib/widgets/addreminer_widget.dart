...
class TimePicker extends StatefulWidget {
  ...
  const TimePicker({Key? key, required this.dateText, required this.timeInfo})
      : super(key: key);
...
class _SaveButtonState extends State<SaveButton> {
  ...
  Future addReminder(
      String reminderText, String startingTime, String finishingTime) async {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: TextButton(
        onPressed: () {
          list.shuffle();
          addReminder(reminderProvider.reminderMessage,
              reminderProvider.startTime, reminderProvider.finishingTime);

          DateTime scheduledStartingDate = DateFormat("yyyy-MM-dd hh:mm:ss").parse(
              "${now.year}-${now.month}-${now.day} ${reminderProvider.startTime}:00");

          DateTime scheduledFinishingDate = DateFormat("yyyy-MM-dd hh:mm:ss").parse(
